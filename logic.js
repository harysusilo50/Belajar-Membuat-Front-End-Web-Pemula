...
function template(id, judul, penulis, tahun, deskripsi, keterangan) {
    ...
    if (keterangan == true) {
        isComplete = `<span class="badge bg-secondary mb-4 fw-normal">Selesai Dibaca</span>`;
        button = `<button class="btn btn-warning text-white" onclick="updateData(${id})">Pindahkan <i class="bi bi-dash-circle"></i></button>`
    } else {
        isComplete = `<span class="badge bg-warning mb-4 fw-normal">Belum Selesai Dibaca</span>`;
